...
The generated `/etc/nixos/flake.nix` follows the running host's current NixOS release line by default, so a 25.05 system stays on `nixos-25.05` instead of silently jumping to 26.x or unstable during bootstrap. If you need a different base explicitly, set `NIXPI_NIXPKGS_FLAKE_URL` before running bootstrap.
