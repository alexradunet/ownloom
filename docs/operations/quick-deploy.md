...
The generated host flake also follows the machine's current NixOS release line by default. That prevents bootstrap from silently jumping a `25.05` host to `26.x` or unstable while NixPI is layered on top.
