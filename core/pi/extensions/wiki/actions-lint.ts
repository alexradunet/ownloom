...
import type { ActionResult, BacklinksData, LintIssue, RegistryData } from "./types.js";
...
export function handleWikiLint(wikiRoot: string, mode = "all"): ActionResult {
	...
	if (mode === "links" || mode === "all") issues.push(...lintLinks(pages, registry));
	if (mode === "orphans" || mode === "all") issues.push(...lintOrphans(registry, backlinks));
	if (mode === "frontmatter" || mode === "all") issues.push(...lintFrontmatter(pages));
	if (mode === "duplicates" || mode === "all") issues.push(...lintDuplicates(registry));
	if (mode === "coverage" || mode === "all") issues.push(...lintCoverage(registry, backlinks));
	if (mode === "staleness" || mode === "all") issues.push(...lintStaleness(registry));
