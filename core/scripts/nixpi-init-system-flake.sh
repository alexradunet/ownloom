...
if [ ! -f "$NIXOS_DIR/configuration.nix" ]; then
  echo "missing $NIXOS_DIR/configuration.nix; install finalization expects a standard NixOS configuration entrypoint" >&2
  exit 1
fi

if [ ! -f "$NIXOS_DIR/hardware-configuration.nix" ]; then
  echo "missing $NIXOS_DIR/hardware-configuration.nix; install finalization expects standard generated NixOS hardware config" >&2
  exit 1
fi
...
should_write_system_flake() {
  ...
  if grep -q '^# Generated by NixPI install finalization$' "$FLAKE_FILE" \
    || grep -q '^# Generated by NixPI bootstrap$' "$FLAKE_FILE"; then
    return 0
  fi
...
# Generated by NixPI install finalization
{
